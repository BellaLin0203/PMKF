...
function renderRecords(){if(!data.records.length){recordList.innerHTML='<div class="empty">目前沒有維護紀錄</div>';return}const s=[...data.records].sort((a,b)=>(b.date||"").localeCompare(a.date||""));recordList.innerHTML='<div class="device-grid">'+s.map(r=>`<div class="device-card"><div class="device-card-header"><div><h3>${esc(deviceName(r.deviceId))}</h3><div class="device-code">${esc(r.partId?partName(r.partId):"未指定零件")}</div></div><span class="badge">${esc(r.date||"-")}</span></div><div class="device-card-body"><div class="device-info"><span>維護內容</span><strong>${esc(r.desc)}</strong></div><div class="device-info"><span>維護人員</span><strong>${esc(r.owner||"-")}</strong></div></div><div class="device-card-actions"><button class="small danger" onclick="deleteRecord('${r.id}')">刪除</button></div></div>`).join('')+'</div>';}function recordsTable(list,ops){return '';}
