...
function renderPartTitle(){const current=selectedDeviceId||partDevice.value;const d=data.devices.find(x=>x.id===current);selectedDeviceTitle.innerHTML=d?`零件管理： <span>${esc(d.name)}</span>`:"零件管理"}
function renderParts(){const current=partDevice.value||selectedDeviceId;selectedDeviceId=current;renderPartTitle();const list=current?data.parts.filter(p=>p.deviceId===current):data.parts;if(!list.length){partList.innerHTML='<div class="empty" style="margin-top:14px">目前沒有零件資料</div>';return}partList.innerHTML='<div class="part-cards">'+list.map(p=>`<div class="part-card"><div class="part-header"><div class="part-name">${esc(p.name)}</div><div>${esc(deviceName(p.deviceId))}</div></div><div class="part-info"><div class="part-item"><div class="label">Part Number</div><div>${esc(p.number||'-')}</div></div><div class="part-item"><div class="label">保養週期</div><div>${p.cycle?p.cycle+' 天':'-'}</div></div></div><div class="part-actions"><button class="small primary" onclick="editPart('${p.id}')">✎ 編輯</button><button class="small danger" onclick="deletePart('${p.id}')">🗑 刪除</button></div></div>`).join('')+'</div>'}
...
function showMainTab(name,btn){["home","devices","records","due"].forEach(t=>document.getElementById("page-"+t).classList.add("hidden"));document.getElementById("page-"+name).classList.remove("hidden");document.querySelectorAll("main > .tabs .tab").forEach(x=>x.classList.remove("active"));const map={home:0,devices:1,records:2,due:3};(btn||document.querySelectorAll("main > .tabs .tab")[map[name]]).classList.add("active");render()}function showDeviceSubTab(name,btn){document.querySelectorAll(".subtab").forEach(x=>x.classList.remove("active"));const map={"device-list":0,"part-list":1};(btn||document.querySelectorAll(".subtab")[map[name]]).classList.add("active");const panel=document.getElementById("device-sub-"+name);if(panel&&window.matchMedia("(max-width: 760px)").matches){panel.scrollIntoView({behavior:"smooth",block:"start"})}render()}
